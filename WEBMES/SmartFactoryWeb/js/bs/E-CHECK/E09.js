...
function E09PKeydown(event) {
    ...
    if (event.keyCode == "13") {
        ...
        $.ajax({
            method: "post",
            url: "../../../controller/E-CHECK/E09.ashx",
...
function Add() {
    ...
    $.ajax({
        ...
        url: "../../../controller/E-CHECK/E09.ashx",
        data: {
            isAutoAssy: isAutoAssy, ModelName: ModelName, user: user, ProjectName: ProjectName, PdlineName: PdlineName, Frequency: Frequency, Check_Name: Check_Name, Check_Item: Check_Item, Unit: Unit, Check_Qty: Check_Qty, Upper: Upper, Floor: Floor, funcName: 'Add'
        },
        dataType: "json",
        async: false,
        success: function (res) {
...
function Show() {
    ...
    var url = "../../../controller/E-CHECK/E09.ashx?funcName=show&&Frequency=" + Frequency + "&&Check_Name=" + Check_Name + "&&Check_Item=" + Check_Item + "&&ProjectNo=" + ProjectNo + "&&PdlineName=" + PdlineName + "&&ModelName=" + ModelName + "&&ProjectName=" + ProjectName;
    ...
    $('#table').bootstrapTable({
        ...
        { field: 'FREQUENCY', title: "点检频率", align: 'center', width: 100, },
            { field: 'EMP_NAME', title: "创建人", align: 'center', width: 100, },
            { field: 'CREATE_DATE', title: "创建时间", align: 'center', width: 100, },
            { field: 'EMP_NAME1', title: "修改人", align: 'center', width: 100, },
    ...
    'click #delete': function (e, value, row, index) {
        ...
        swal({
            ...
            $.ajax({
                url: "../../../controller/E-CHECK/E09.ashx",
                type: "post",
                data: { funcName: 'del', ModelName: row.MODEL_NAME, user: user, NUMBER_ID: row.PROJECT_ID, ProjectName: row.PROJECT_NAME, PdlineName: row.PDLINE_NAME, Type2: row.TYPE2, Type3: row.TYPE3, Type5: row.TYPE5, Type6: row.TYPE6, Type4: row.TYPE4, Type7: row.TYPE7 },
                dataType: "json",
                async: false,
                success: function (res) {
                    var error_msg = "";
...
function importExcel() {
    ...
    $.ajax({
        url: "../../../controller/E-CHECK/E09.ashx?funcName=excel&user=" + user + "&Path=" + files + "",
        type: "POST",
        data: data,
        async: false,
        contentType: false,
