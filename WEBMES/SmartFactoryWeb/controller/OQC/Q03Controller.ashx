<%@ WebHandler Language="C#" Class="Q03Controller" %>
...
public class Q03Controller : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        string JigNO = context.Request["JigNO"];
        ...
        switch (funcName)
        {
            ...
            case "insertNgtmp":
                ...
                break;
            case "checkOK":
                rtnValue = checkOK(lotnum, lotid, user, ouser, JigNO, checknum, ddltype,stepid);
                break;
            case "checkouser":
    ...
    //获取人員基本信息
    private string GetUser(string User_ID)
    {
        string User = "";
    ...
    {
        string rtn = ""; string msgshow = "";  string mo_type = "";
        lotid = lotid.ToUpper().Trim();
        int qty = 0;       
        string sqlmmk1 = "SELECT IS_LOCK FROM LOCK_TYPE WHERE TYPE_NAME='OQCD' AND LOT_ID='" + lotid + "'";
        DataTable dtmmk1 = PubClass.getdatatableMES(sqlmmk1);
        if (dtmmk1.Rows.Count > 0)
        {
            string LNGV = "true";
            rtn = "[{\"FLAG\":\"N\",\"MSG\":\"此LOT已被過賬鎖定，請稍等\",\"LNGV\":\"" + LNGV + "\"}]";
            return rtn;
        }
        ...
        if (dtlot_tmp.Rows.Count > 0)
        {
            ...
            for (int m = 0; m < dtlot_tmp.Rows.Count; m++)
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    {
                        rtn = "[{\"FLAG\":\"N\",\"MSG\":\"" + msgshow + "此批产品已被HOLD\"}]";
                        return rtn;
                    }
    ...
    private string insertNgtmp(string ngnum, string lotid, string ngremark, string ngcodeid, string lotnum, string ddltype, string checknum, string nglot, string mlotnum)
    {
        ...
        String sql1=@"INSERT INTO SD_TMP_OQCNG(ELOT_ID,LOT_ID,ERROR_CODE,NG_NUM,LINE_ID,REMARK) 
                                           VALUES('" + lotid + "','" +nglot+ "','" + ngcodeid + "','" + ngnum + "','" + dt.Rows[0]["Line_id"].ToString() + "','" + ngremark + "')";
    ...
    private string checkOK(string lotnum,string lotid, string user, string ouser, string JigNO, string checknum, string ddltype,string stepid)
    {
        string rtn = "";      
        ...
        if (dtCheckStepCurrent.Rows.Count > 0)
        {
            ...
            {
                if (dtCheckStepCurrent.Rows[i]["step_current"].ToString() != "" + stepid + "")    //      待修改
                {
        ...
        if (JigNO.Length > 10)
        {
            rtn = "[{\"FLAG\":\"NgJig\",\"MSG\":\"治具編碼長度不能超過10碼!\"}]";
            return rtn;
        }
        string sqlmmk1 = "SELECT IS_LOCK FROM LOCK_TYPE WHERE TYPE_NAME='OQCD' AND LOT_ID='" + lotid + "'";
        DataTable dtmmk1 = PubClass.getdatatableMES(sqlmmk1);
        if (dtmmk1.Rows.Count == 0)
        {
            string sqlup1 = "INSERT INTO LOCK_TYPE (TYPE_NAME,IS_LOCK,LOT_ID) VALUES ('OQCD','Y','" + lotid + "')";
        ...
        string sqlup2 = " DELETE FROM LOCK_TYPE WHERE TYPE_NAME='OQCD' AND LOT_ID='" + lotid + "'";
