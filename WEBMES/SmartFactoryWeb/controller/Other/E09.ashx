<%@ WebHandler Language="C#" Class="E09" %>
using System;
using System.Web;
using System.Data;
...
public class E09 : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        switch (funcName)
        {
            ...
            case "ShowProject":
                ...
                break;
            case "ShowDown":
                rtnValue = ShowDown(PdlineName, ModelName, dataType, datetimepicker);
                break;
            case "ShowBuNo":
            ...
            case "UpdateTemplateInfo1":
                rtnValue = UpdateTemplateInfo(ModelName, Remark, Section_Name, isAutoAssy, TemplateQty, Pro, DnNo, user, ID, Type1, Type2, Type3, Type4, Type5, Type6, Type7, Type8, Type9, Type10, Type11, Type12, Check_No, Check_Name, Check_Item, Unit, Check_Qty, Upper, Floor);
                break;
            case "Save":
    ...
    public string ShowDown(string PdlineName, string ModelName, string dataType, string datetimepicker)
    {
        string SQL = $"SELECT * FROM SAJET.SYS_PROJECT_MODEL_BASE WHERE MODEL_NAME='{ModelName}' AND PDLINE_NAME='{PdlineName}' AND STATUS='Y'";
        DataTable dt = PubClass.getdatatableMES(SQL);
        if (dt.Rows.Count == 0)
        {
            return "[{\"ERR_CODE\":\"Y\",\"ERR_MSG\":\"NG,机种线体未维护唯一标题信息!\"}]";
        }
        string PROJECT_ID = dt.Rows[0]["PROJECT_ID"].ToString();

        SQL = $@"SELECT DISTINCT FREQUENCY FROM   SAJET.SYS_PROJECT_MODEL_BASE WHERE PROJECT_ID='{PROJECT_ID}'AND MODEL_NAME='{ModelName}' AND PDLINE_NAME='{PdlineName}'";
        dt = PubClass.getdatatableMES(SQL);
        SQL = $@"SELECT CHECK_TIME FROM  SAJET.SYS_CHECK_PROJECT_TITLE WHERE PROJECT_ID='{PROJECT_ID}' AND  FREQUENCY='{dt.Rows[0][0].ToString()}'ORDER BY SEQ";
        dt = PubClass.getdatatableMES(SQL);
        if (dt.Rows.Count == 0)
        {
            return "[{\"ERR_CODE\":\"Y\",\"ERR_MSG\":\"未查询到标题信息\"}]";
        }
        int Lenght = dt.Rows.Count; int L = 0;
        if (dataType == "白班")
        {
            L = 0;
        }
        ...
        string tableHtmlEnd = "</tr>";
        string MIN = ""; string MAX = ""; int P = 0; int Count = 0; string LOG = "";
        for (int i = 0; i <= dt.Rows.Count - 2; i++)
        {
            MIN = dt.Rows[i]["CHECK_TIME"].ToString().Split(' ')[L].Split('-')[0].Split(':')[0];
            MAX = dt.Rows[i]["CHECK_TIME"].ToString().Split(' ')[L].Split('-')[1].Split(':')[0];
            if (int.Parse(MAX) == 0)
            {
                Count = int.Parse(MAX) + 24 - int.Parse(MIN);

            }
            else
            {
                Count = int.Parse(MAX) - int.Parse(MIN);
            }
            P = P + Count;
            LOG += $"<td colspan='{Count}'>{dt.Rows[i]["CHECK_TIME"].ToString().Split(' ')[L]}</td>";
        }
        LOG += $"<td colspan='1'>{dt.Rows[dt.Rows.Count - 1]["CHECK_TIME"].ToString().Split(' ')[L]}</td><td colspan='1'>备注</td>";
        LOG += $"</tr>";
        string LOG1 = $"<tr style=text-align:center><td colspan='1'>机种名称</td><td colspan='{P-2}'>{ModelName}</td><td colspan='3'>日期</td><td colspan='3'>{datetimepicker}</td><td colspan='2'>版本</td><td colspan='2'>A</td></tr>";
        string LOG2 = $"<tr style=text-align:center><td colspan='1'>线别</td><td colspan='{P-2}'>{PdlineName}</td><td colspan='3'>班别</td><td colspan='3'>{dataType}</td><td colspan='2'>成品料号</td><td colspan='2'></td></tr>";
        //string LOG3 = $"<tr style=text-align:center><td colspan='1'>制程名称</td><td colspan='2'>检查事项-Check Point</td><td colspan='1'>抽测数量</td><td colspan='1'>管控上限</td><td colspan='1'>管控下限</td><td colspan='{P + 2}'>巡回记录</td></tr>";

        string LOG3 = $"<tr style=text-align:center><td colspan='1'>制程名称</td><td colspan='1'>工站序号</td><td colspan='1'>点检制程名称</td><td colspan='1'>检查事项-Check Point</td><td colspan='1'>抽测数量</td><td colspan='1'>管控上限</td><td colspan='1'>管控下限</td>"+LOG;




        string LOG4 = "<tr>"; string LOG5 = ""; string LOG51 = "";string LOG6 = "";

        //SQL = $"SELECT * FROM SAJET.SYS_PROJECT_PDLINE_BASE  WHERE MODEL_NAME='{ModelName}' AND PDLINE_NAME='{PdlineName}' ORDER BY PROJECT_NAME ";
        SQL = $"SELECT  DISTINCT A.PROJECT_ID,A.PROJECT_NO FROM SAJET.SYS_PROJECT_MODEL_BASE A,SAJET.SYS_ECHECK_PROJECT_BASE B,SAJET.SYS_CHECK_PROJECT_TITLE  C WHERE A.MODEL_NAME='{ModelName}' AND A.PDLINE_NAME='{PdlineName}'AND A.PROJECT_ID=C.PROJECT_ID AND A.PROJECT_ID=B.PROJECT_ID AND A.CHECK_ITEM=B.CHECK_ITEM AND A.CHECK_NAME=B.CHECK_NAME  ORDER BY A.PROJECT_NO";
        dt = PubClass.getdatatableMES(SQL);
        //for (int K = 0; K <= 2; K++)
        for (int K = 0; K <= dt.Rows.Count - 1; K++)
        {
            LOG5 = "";  LOG51 = "";
            SQL = $"SELECT   A.PROJECT_NO,A.PROJECT_NAME,B.CHECK_NAME,B.CHECK_NO,A.CHECK_ITEM,A.PROJECT_ID,A.CHECK_QTY,A.UPPER,A.FLOOR FROM SAJET.SYS_PROJECT_MODEL_BASE A,SAJET.SYS_ECHECK_PROJECT_BASE B WHERE A.MODEL_NAME='{ModelName}' AND A.PDLINE_NAME='{PdlineName}'AND A.PROJECT_ID='{dt.Rows[K]["PROJECT_ID"].ToString()}' AND A.PROJECT_NO='{dt.Rows[K]["PROJECT_NO"].ToString()}' AND A.PROJECT_ID=B.PROJECT_ID AND A.CHECK_NAME=B.CHECK_NAME AND A.CHECK_ITEM=B.CHECK_ITEM  ORDER BY A.PROJECT_NAME,B.CHECK_NO";
            DataTable DTCOUNT = PubClass.getdatatableMES(SQL);
            LOG51 = $"<tr><td rowspan='{DTCOUNT.Rows.Count.ToString()}' colspan='1'>{DTCOUNT.Rows[0]["PROJECT_NAME"].ToString()}</td>";
            for (int T = 0; T <= DTCOUNT.Rows.Count - 1; T++)
            {
                string TYPE1 = DTCOUNT.Rows[T]["CHECK_NO"].ToString() + "." + DTCOUNT.Rows[T]["CHECK_ITEM"].ToString();
                LOG5 += $"<td  colspan='1'>{DTCOUNT.Rows[T]["PROJECT_NO"].ToString()}</td><td  colspan='1'>{DTCOUNT.Rows[T]["CHECK_NAME"].ToString()}</td><td colspan='1'>{TYPE1}</td><td colspan='1'>{DTCOUNT.Rows[T]["CHECK_QTY"].ToString()}</td><td colspan='1'>{DTCOUNT.Rows[T]["UPPER"].ToString()}</td><td colspan='1'>{DTCOUNT.Rows[T]["FLOOR"].ToString()}</td>";




                SQL = $@"SELECT DISTINCT FREQUENCY FROM   SAJET.SYS_PROJECT_MODEL_BASE WHERE PROJECT_ID='{DTCOUNT.Rows[T]["PROJECT_ID"].ToString()}'AND MODEL_NAME='{ModelName}' AND PDLINE_NAME='{PdlineName}' AND PROJECT_NO='{DTCOUNT.Rows[T]["PROJECT_NO"].ToString()}'";
                DataTable dt1 = PubClass.getdatatableMES(SQL);
                SQL = $@"SELECT CHECK_TIME FROM  SAJET.SYS_CHECK_PROJECT_TITLE WHERE PROJECT_ID='{DTCOUNT.Rows[T]["PROJECT_ID"].ToString()}' AND  FREQUENCY='{dt1.Rows[0][0].ToString()}'ORDER BY SEQ";
                dt1 = PubClass.getdatatableMES(SQL);




                for (int Q = 0; Q <= dt1.Rows.Count - 2; Q++)
                {
                    string TYPE = "TYPE" + (Q + 1).ToString();
                    MIN = dt1.Rows[Q]["CHECK_TIME"].ToString().Split(' ')[L].Split('-')[0].Split(':')[0];
                    MAX = dt1.Rows[Q]["CHECK_TIME"].ToString().Split(' ')[L].Split('-')[1].Split(':')[0];
                    if (int.Parse(MAX) == 0)
                    {
                        Count = int.Parse(MAX) + 24 - int.Parse(MIN);
                    ...
                    SQL = $"SELECT B.{TYPE} FROM SAJET.G_CHECK_WORK_BASE A,SAJET.G_CHECK_WORK_BASE_INFO B WHERE A.WORK_DATE='{datetimepicker}' AND A.WORK_TYPE='{dataType}' AND A.PDLINE_NAME='{PdlineName}' " +
                          $"AND A.MODEL_NAME='{ModelName}' AND A.PROJECT_NO='{DTCOUNT.Rows[T]["PROJECT_NO"].ToString()}' AND A.WORK_NO=B.WORK_NO AND B.CHECK_NAME='{DTCOUNT.Rows[T]["CHECK_NAME"].ToString()}' AND B.CHECK_ITEM='{DTCOUNT.Rows[T]["CHECK_ITEM"].ToString()}' AND B.{TYPE} IS NOT NULL ";
                    DataTable dt11 = PubClass.getdatatableMES(SQL);
                    if (dt11.Rows.Count > 0)
                    {
                        LOG5 += $"<td colspan='{Count}'>{dt11.Rows[0][0].ToString()}</td>";
                    }
                    else
                    {
                        LOG5 += $"<td colspan='{Count}'></td>";
                    }
                }
                MIN = dt1.Rows[dt1.Rows.Count - 1]["CHECK_TIME"].ToString().Split(' ')[L].Split('-')[0].Split(':')[0];
                MAX = dt1.Rows[dt1.Rows.Count - 1]["CHECK_TIME"].ToString().Split(' ')[L].Split('-')[1].Split(':')[0];
                Count = int.Parse(MAX) - int.Parse(MIN);
                string LTYPE = "TYPE" + (dt1.Rows.Count - 1).ToString();
                SQL = $"SELECT B.{LTYPE} FROM SAJET.G_CHECK_WORK_BASE A,SAJET.G_CHECK_WORK_BASE_INFO B WHERE A.WORK_DATE='{datetimepicker}' AND A.WORK_TYPE='{dataType}' AND A.PDLINE_NAME='{PdlineName}' " +
                          $"AND A.MODEL_NAME='{ModelName}'AND A.PROJECT_NO='{DTCOUNT.Rows[T]["PROJECT_NO"].ToString()}' AND A.WORK_NO=B.WORK_NO AND B.CHECK_NAME='{DTCOUNT.Rows[T]["CHECK_NAME"].ToString()}' AND B.CHECK_ITEM='{DTCOUNT.Rows[T]["CHECK_ITEM"].ToString()}' AND B.{LTYPE} IS NOT NULL ";
                DataTable dt112 = PubClass.getdatatableMES(SQL);
                if (dt112.Rows.Count > 0)
                {
                    LOG5 += $"<td colspan='1'>{dt112.Rows[0][0].ToString()}</td><td colspan='1'></td>";
                }
                else
                {
                    LOG5 += $"<td colspan='1'></td><td colspan='1'></td>";
                }
                LOG5 += $"</tr>";
        ...
        tableHtml += LOG1 + LOG2 + LOG3  + LOG6 + "</table></body></html>";
    ...
    public string UpdateTemplateInfoL(string PdlineName, string ModelName, string Remark, string Section_Name, string isAutoAssy, string TemplateQty, string Pro, string DN_NO, string user, string ID, string Type1, string Type2, string Type3, string Type4, string Type5, string Type6, string Type7, string Type8, string Type9, string Type10, string Type11, string Type12, string Check_No, string Check_Name, string Check_Item, string Unit, string Check_Qty, string Upper, string Floor)
    {
        ...
        for (int L = 0; L <= int.Parse(TemplateQty); L++)
        {
            ...
            if (L == 1)
            {
                ...
                if (Type2 != "NA" && Type2 != "N/A" && Type2 != "" && Type2 != null)
                {
                    ...
                    if (num3 != 0)
                    {
                        if (Type2.Split('/').Length != 2)
                        {
                            return "[{\"ERR_CODE\":\"Y\",\"ERR_MSG\":\"NG,输入值未用/隔开!\"}]";
                        }
                        try
                        {
                            int.Parse(Type2.Split('/')[1]);
                        }
                        catch
                        {
                            return "[{\"ERR_CODE\":\"Y\",\"ERR_MSG\":\"NG,/后请输入数字!\"}]";
                        }
                        if (int.Parse(Check_Qty) > int.Parse(Type2.Split('/')[1]))
                        {
                            return "[{\"ERR_CODE\":\"Y\",\"ERR_MSG\":\"NG,抽检数量小于需要抽检数量!\"}]";
                        }
                    }
            ...
            if (L == 2)
            {
                if (Type3 != "NA" && Type3 != "N/A" && Type3 != "" && Type3 != null)
                {
                    ...
                    if (num3 != 0)
                    {
                        if (Type3.Split('/').Length != 2)
                        {
                            return "[{\"ERR_CODE\":\"Y\",\"ERR_MSG\":\"NG,输入值未用/隔开!\"}]";
                        }
                        try
                        {
                            int.Parse(Type3.Split('/')[1]);
                        }
                        catch
                        {
                            return "[{\"ERR_CODE\":\"Y\",\"ERR_MSG\":\"NG,/后请输入数字!\"}]";
                        }
                        if (int.Parse(Check_Qty) > int.Parse(Type3.Split('/')[1]))
                        {
                            return "[{\"ERR_CODE\":\"Y\",\"ERR_MSG\":\"NG,抽检数量小于需要抽检数量!\"}]";
                        }
                    }
            ...
            if (L == 3)
            {
                if (Type4 != "NA" && Type4 != "N/A" && Type4 != "" && Type4 != null)
                {
                    ...
                    if (num3 != 0)
                    {
                        if (Type4.Split('/').Length != 2)
                        {
                            return "[{\"ERR_CODE\":\"Y\",\"ERR_MSG\":\"NG,输入值未用/隔开!\"}]";
                        }
                        try
                        {
                            int.Parse(Type4.Split('/')[1]);
                        }
                        catch
                        {
                            return "[{\"ERR_CODE\":\"Y\",\"ERR_MSG\":\"NG,/后请输入数字!\"}]";
                        }
                        if (int.Parse(Check_Qty) > int.Parse(Type4.Split('/')[1]))
                        {
                            return "[{\"ERR_CODE\":\"Y\",\"ERR_MSG\":\"NG,抽检数量小于需要抽检数量!\"}]";
                        }
                    }
            ...
            if (L == 4)
            {
                if (Type5 != "NA" && Type5 != "N/A" && Type5 != "" && Type5 != null)
                {
                    ...
                    if (num3 != 0)
                    {
                        if (Type5.Split('/').Length != 2)
                        {
                            return "[{\"ERR_CODE\":\"Y\",\"ERR_MSG\":\"NG,输入值未用/隔开!\"}]";
                        }
                        try
                        {
                            int.Parse(Type5.Split('/')[1]);
                        }
                        catch
                        {
                            return "[{\"ERR_CODE\":\"Y\",\"ERR_MSG\":\"NG,/后请输入数字!\"}]";
                        }
                        if (int.Parse(Check_Qty) > int.Parse(Type5.Split('/')[1]))
                        {
                            return "[{\"ERR_CODE\":\"Y\",\"ERR_MSG\":\"NG,抽检数量小于需要抽检数量!\"}]";
                        }
                    }
            ...
            if (L == 5)
            {
                if (Type6 != "NA" && Type6 != "N/A" && Type6 != "" && Type1 != null)
                {
                    decimal.TryParse(Check_Qty, out num3);

                    if (num3 != 0)
                    {
                        if (Type6.Split('/').Length != 2)
                        {
                            return "[{\"ERR_CODE\":\"Y\",\"ERR_MSG\":\"NG,输入值未用/隔开!\"}]";
                        }
                        try
                        {
                            int.Parse(Type6.Split('/')[1]);
                        }
                        catch
                        {
                            return "[{\"ERR_CODE\":\"Y\",\"ERR_MSG\":\"NG,/后请输入数字!\"}]";
                        }
                        if (int.Parse(Check_Qty) > int.Parse(Type6.Split('/')[1]))
                        {
                            return "[{\"ERR_CODE\":\"Y\",\"ERR_MSG\":\"NG,抽检数量小于需要抽检数量!\"}]";
                        }
                    }
            ...
            if (L == 6)
            {
                if (Type7 != "NA" && Type7 != "N/A" && Type7 != "" && Type7 != null)
                {
                    ...
                    if (num3 != 0)
                    {
                        if (Type7.Split('/').Length != 2)
                        {
                            return "[{\"ERR_CODE\":\"Y\",\"ERR_MSG\":\"NG,输入值未用/隔开!\"}]";
                        }
                        try
                        {
                            int.Parse(Type7.Split('/')[1]);
                        }
                        catch
                        {
                            return "[{\"ERR_CODE\":\"Y\",\"ERR_MSG\":\"NG,/后请输入数字!\"}]";
                        }
                        if (int.Parse(Check_Qty) > int.Parse(Type7.Split('/')[1]))
                        {
                            return "[{\"ERR_CODE\":\"Y\",\"ERR_MSG\":\"NG,抽检数量小于需要抽检数量!\"}]";
                        }
                    }
            ...
            if (L == 7)
            {
                if (Type8 != "NA" && Type8 != "N/A" && Type8 != "" && Type8 != null)
                {
                    ...
                    if (num3 != 0)
                    {
                        if (Type8.Split('/').Length != 2)
                        {
                            return "[{\"ERR_CODE\":\"Y\",\"ERR_MSG\":\"NG,输入值未用/隔开!\"}]";
                        }
                        try
                        {
                            int.Parse(Type8.Split('/')[1]);
                        }
                        catch
                        {
                            return "[{\"ERR_CODE\":\"Y\",\"ERR_MSG\":\"NG,/后请输入数字!\"}]";
                        }
                        if (int.Parse(Check_Qty) > int.Parse(Type8.Split('/')[1]))
                        {
                            return "[{\"ERR_CODE\":\"Y\",\"ERR_MSG\":\"NG,抽检数量小于需要抽检数量!\"}]";
                        }
                    }
            ...
            if (L == 8)
            {
                if (Type9 != "NA" && Type9 != "N/A" && Type9 != "" && Type9 != null)
                {
                    ...
                    if (num3 != 0)
                    {
                        if (Type9.Split('/').Length != 2)
                        {
                            return "[{\"ERR_CODE\":\"Y\",\"ERR_MSG\":\"NG,输入值未用/隔开!\"}]";
                        }
                        try
                        {
                            int.Parse(Type9.Split('/')[1]);
                        }
                        catch
                        {
                            return "[{\"ERR_CODE\":\"Y\",\"ERR_MSG\":\"NG,/后请输入数字!\"}]";
                        }
                        if (int.Parse(Check_Qty) > int.Parse(Type9.Split('/')[1]))
                        {
                            return "[{\"ERR_CODE\":\"Y\",\"ERR_MSG\":\"NG,抽检数量小于需要抽检数量!\"}]";
                        }
                    }
            ...
            if (L == 9)
            {
                if (Type10 != "NA" && Type10 != "N/A" && Type10 != "" && Type10 != null)
                {
                    ...
                    if (num3 != 0)
                    {
                        if (Type10.Split('/').Length != 2)
                        {
                            return "[{\"ERR_CODE\":\"Y\",\"ERR_MSG\":\"NG,输入值未用/隔开!\"}]";
                        }
                        try
                        {
                            int.Parse(Type10.Split('/')[1]);
                        }
                        catch
                        {
                            return "[{\"ERR_CODE\":\"Y\",\"ERR_MSG\":\"NG,/后请输入数字!\"}]";
                        }
                        if (int.Parse(Check_Qty) > int.Parse(Type10.Split('/')[1]))
                        {
                            return "[{\"ERR_CODE\":\"Y\",\"ERR_MSG\":\"NG,抽检数量小于需要抽检数量!\"}]";
                        }
                    }
            ...
            if (L == 10)
            {
                if (Type11 != "NA" && Type11 != "N/A" && Type11 != "" && Type11 != null)
                {
                    ...
                    if (num3 != 0)
                    {
                        if (Type11.Split('/').Length != 2)
                        {
                            return "[{\"ERR_CODE\":\"Y\",\"ERR_MSG\":\"NG,输入值未用/隔开!\"}]";
                        }
                        try
                        {
                            int.Parse(Type11.Split('/')[1]);
                        }
                        catch
                        {
                            return "[{\"ERR_CODE\":\"Y\",\"ERR_MSG\":\"NG,/后请输入数字!\"}]";
                        }
                        if (int.Parse(Check_Qty) > int.Parse(Type11.Split('/')[1]))
                        {
                            return "[{\"ERR_CODE\":\"Y\",\"ERR_MSG\":\"NG,抽检数量小于需要抽检数量!\"}]";
                        }
                    }
            ...
            if (L == 11)
            {
                if (Type12 != "NA" && Type12 != "N/A" && Type12.Split('/')[0] != "" && Type12.Split('/')[0] != null)
                {
                    decimal.TryParse(Check_Qty, out num3);

                    if (num3 != 0)
                    {
                        if (Type12.Split('/').Length != 2)
                        {
                            return "[{\"ERR_CODE\":\"Y\",\"ERR_MSG\":\"NG,输入值未用/隔开!\"}]";
                        }
                        try
                        {
                            int.Parse(Type12.Split('/')[1]);
                        }
                        catch
                        {
                            return "[{\"ERR_CODE\":\"Y\",\"ERR_MSG\":\"NG,/后请输入数字!\"}]";
                        }
                        if (int.Parse(Check_Qty) > int.Parse(Type12.Split('/')[1]))
                        {
                            return "[{\"ERR_CODE\":\"Y\",\"ERR_MSG\":\"NG,抽检数量小于需要抽检数量!\"}]";
                        }
                    }
        ...
        if (isAutoAssy == "Y")
        {
            ...
            string Email = "";
            string SQL = $"SELECT * FROM  SAJET.SYS_CHECK_SECTOR_CONFIG WHERE SECTON_NAME='{Section_Name}' AND CHECK_TYPE='2'AND STATUS='异常'";
            DataTable dt1 = PubClass.getdatatableMES(SQL);
            if (dt1.Rows.Count == 0)
            {
            ...
            string sSQL1 = $"SELECT * FROM  SAJET.SYS_EMP WHERE EMP_ID IN(SELECT EMP_ID FROM  SAJET.SYS_CHECK_SECTOR_CONFIG WHERE SECTON_NAME='{Section_Name}' AND CHECK_TYPE='2'AND STATUS='异常')AND EMAIL IS NOT NULL";
    ...
    public string UpdateTemplateInfo(string Model_name, string Remark, string Section_Name, string isAutoAssy, string TemplateQty, string Pro, string DN_NO, string user, string ID, string Type1, string Type2, string Type3, string Type4, string Type5, string Type6, string Type7, string Type8, string Type9, string Type10, string Type11, string Type12, string Check_No, string Check_Name, string Check_Item, string Unit, string Check_Qty, string Upper, string Floor)
    {
        decimal num = 0; decimal num1 = 0; double MAX = 0; double MIN = 0; string Flag = "N";
        string userEmpId = getEmpIdByNo(user);
        if (userEmpId.Length == 0)
        ...
        for (int L = 0; L <= int.Parse(TemplateQty); L++)
        {
            ...
            if (L == 1)
            {
                if (Type2 != "NA" && Type2 != "N/A" && Type2 != "" && Type2 != null)
                {
                    ...
                    if (Flag == "N") { if (Type2.Split('/')[0] == "OK" || Type2 == "NG") { } else { return "[{\"ERR_CODE\":\"Y\",\"ERR_MSG\":\"NG,输入值请输入OK或者NG!\"}]"; } }
        ...
        if (isAutoAssy == "Y")
        {
            ...
            if (dt1.Rows.Count > 0)
            {
                ...
                Email = Email.Substring(0, Email.Length - 1);
                sSQL = $@"SELECT TITLE FROM SAJET.SYS_CHECK_TEMPLATE_TITLE WHERE PROJECT_NAME ='{Pro}' ORDER BY SEQ";
                dt = PubClass.getdatatableMES(sSQL);
                int QTY = dt.Rows.Count;
                ...
                titleMessage += "</tr>";
                string detailMessage = $@"";
                sSQL = $@"SELECT * FROM  SAJET.G_CHECK_BASE_INFO WHERE NUMBER_ID='{ID}' AND WORK_NO='{DN_NO}'";
                dt = PubClass.getdatatableMES(sSQL);
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    detailMessage += $@"<tr style='font-size:13px;'>";
        ...
        sSQL = $@"UPDATE SAJET.G_CHECK_BASE_INFO SET  REMARK='{Remark}',TYPE1='{Type1}',TYPE2='{Type2}',TYPE3='{Type3}',TYPE4='{Type4}',TYPE5='{Type5}',TYPE6='{Type6}',TYPE7='{Type7}',TYPE8='{Type8}',TYPE9='{Type9}',TYPE10='{Type10}',
        TYPE11='{Type11}',TYPE12='{Type12}',UPDATE_DATE=SYSDATE,UPDATE_EMPID='{userEmpId}' WHERE WORK_NO='{DN_NO}' AND NUMBER_ID='{ID}'";
        PubClass.getdatatablenoreturnMES(sSQL);
        return "[{\"ERR_CODE\":\"N\",\"ERR_MSG\":\"OK,点检成功!\"}]";
    }
    ...
    public string GetProcess(string ProjectNo, string ModelName, string type, string user, string datetimepicker, string dataType, string PdlineName, string Section_Name, string ProjectName)
    {
        ...
        SQL = $"SELECT * FROM SAJET.G_CHECK_WORK_BASE WHERE WORK_DATE='{datetimepicker}'  AND PROJECT_NAME='{ProjectName}'AND WORK_TYPE='{dataType}'AND PDLINE_NAME='{PdlineName}' AND MODEL_NAME='{ModelName}' AND PROJECT_NO='{ProjectNo}'";
        dt = PubClass.getdatatableMES(SQL);
        if (dt.Rows.Count == 0)
        {
            ...
            for (int i = 0; i <= dt.Rows.Count - 1; i++)
            {
                ...
                if (dt1.Rows.Count > 0)
                {
                    ...
                    SQL = $"INSERT INTO SAJET.G_CHECK_WORK_BASE_INFO(PROJECT_NO,CHECK_NO,WORK_NO,PROJECT_ID,PROJECT_NAME,PDLINE_NAME,FREQUENCY,CHECK_NAME,CHECK_ITEM,UNIT,CHECK_QTY,UPPER,FLOOR,NUMBER_ID,MODEL_NAME,ENABLED,CREATE_DATE,CREATE_EMPID,CHECK_NO,PROJECT_NO)" +
                    $"SELECT '{ProjectNo}','{CHECK_NO}','{WORK_NO}','{PROJECT_ID}','{ProjectName}','{PdlineName}','{FREQUENCY}','{CHECK_NAME}','{CHECK_ITEM}','{UNIT}','{CHECK_QTY}','{UPPER}','{FLOOR}','{NUMBER_ID}','{ModelName}','Y',SYSDATE,'{userEmpId}','{CHECK_NO}','{ProjectNo}' FROM DUAL";
                    PubClass.getdatatablenoreturnMES(SQL);
                }
            }
        }
        else
        {
            ...
            for (int i = 0; i <= dt.Rows.Count - 1; i++)
            {
                ...
                if (dt1.Rows.Count > 0)
                {
                    ...
                    if (dt1.Rows.Count == 0)
                    {
                        SQL = $"INSERT INTO SAJET.G_CHECK_WORK_BASE_INFO(PROJECT_NO,CHECK_NO,WORK_NO,PROJECT_ID,PROJECT_NAME,PDLINE_NAME,FREQUENCY,CHECK_NAME,CHECK_ITEM,UNIT,CHECK_QTY,UPPER,FLOOR,NUMBER_ID,MODEL_NAME,ENABLED,CREATE_DATE,CREATE_EMPID)" +
                   $"SELECT '{ProjectNo}','{CHECK_NO}','{WORK_NO}','{PROJECT_ID}','{ProjectName}','{PdlineName}','{FREQUENCY}','{CHECK_NAME}','{CHECK_ITEM}','{UNIT}','{CHECK_QTY}','{UPPER}','{FLOOR}','{NUMBER_ID}','{ModelName}','Y',SYSDATE,'{userEmpId}' FROM DUAL";
                        PubClass.getdatatablenoreturnMES(SQL);
                    }
                }
            }
    ...
    {
        string SQL = $"SELECT C.CHECK_NO,A.PROJECT_NAME,A.WORK_DATE,A.WORK_TYPE,A.PROJECT_NAME,B.* FROM SAJET.G_CHECK_WORK_BASE A,SAJET.G_CHECK_WORK_BASE_INFO B, SAJET.SYS_ECHECK_PROJECT_BASE C WHERE A.WORK_NO='{DN_NO}' AND A.WORK_NO=B.WORK_NO AND C.SECTION_NAME IN('ALL','{Section_Name}')AND B.NUMBER_ID=C.NUMBER_ID AND B.ENABLED='Y'";
        DataTable dt = PubClass.getdatatableMES(SQL);
        return JsonConvert.SerializeObject(dt);
    }
