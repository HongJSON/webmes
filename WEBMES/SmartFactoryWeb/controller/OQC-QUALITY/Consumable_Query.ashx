<%@ WebHandler Language="C#" Class="Consumable_Query" %>
using System;
using System.Web;
using System.Data;
...
public class Consumable_Query : IHttpHandler
{
    ...
    public string show(string PdlineName, string ModelName, string Process_Name, string Consumable_Type)
    {
        ...
        string SQL = $"SELECT A.PDLINE_NAME,A.MODEL_NAME,A.PROCESS_NAME,A.CONSUMABLE_TYPE,A.SN,A.REMARK,A.CREATE_DATE,B.EMP_NAME,A.UPDATE_DATE,C.EMP_NAME AS EMP_NAME1 FROM  SAJET.SYS_CONSUMABLE_BASE A,SAJET.SYS_EMP B,SAJET.SYS_EMP C WHERE A.CREATE_USERID=B.EMP_ID(+) AND A.UPDATE_USERID=C.EMP_ID(+)" + sql + " ORDER BY A.CREATE_DATE DESC";
        DataTable dt = PubClass.getdatatableMES(SQL);
