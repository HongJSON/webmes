...
public class E10 : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        switch (funcName)
        {
            ...
            case "show":
                ...
                break;
            case "del":
                rtnValue = del(ModelName, NUMBER_ID, user, ProjectName, PdlineName, Check_Name, Check_Item);
                break;
            case "excel":
    ...
    public string ShowDown(string PdlineName, string ModelName)
    {
        ...
        string SQL = $"SELECT DISTINCT PDLINE_NAME,MODEL_NAME,PROJECT_NAME FROM SAJET.SYS_PROJECT_MODEL_BASE WHERE  1=1 "+sql+"";
        ...
        string LOG1 = $"<tr style=text-align:center><td colspan='1'>线体</td><td colspan='1'>机种</td><td colspan='1'>制程名称</td><td colspan='1'>打印Label</td></tr>";
        string LOG2 = "";
        for(int i=0;i<=dt.Rows.Count-1;i++)
        {
            string LOG3 = dt.Rows[i][0].ToString() + "|" + dt.Rows[i][1].ToString() + "|" + dt.Rows[i][2].ToString();
            LOG2 += $"<tr style=text-align:center><td colspan='1'>{dt.Rows[i][0].ToString()}</td><td colspan='1'>{dt.Rows[i][1].ToString()}</td><td colspan='1'>{dt.Rows[i][2].ToString()}</td><td colspan='1'>{LOG3}</td></tr>";
        }
    ...
    public string del(string ModelName,string NUMBER_ID,string user,string ProjectName,string PdlineName,string Check_Name,string Check_Item)
    {
        ...
        string sSQL = $"UPDATE SAJET.SYS_PROJECT_MODEL_BASE SET UPDATE_EMPID='{userEmpId}',UPDATE_DATE=SYSDATE WHERE PROJECT_ID='{NUMBER_ID}' AND MODEL_NAME='{ModelName}' AND PDLINE_NAME='{PdlineName}' AND  CHECK_NAME='{Check_Name}' AND CHECK_ITEM='{Check_Item}'";
        PubClass.getdatatablenoreturnMES(sSQL);

        sSQL = $"INSERT INTO SAJET.SYS_HT_PROJECT_MODEL_BASE SELECT * FROM SAJET.SYS_PROJECT_MODEL_BASE WHERE PROJECT_ID='{NUMBER_ID}' AND MODEL_NAME='{ModelName}' AND PDLINE_NAME='{PdlineName}' AND  CHECK_NAME='{Check_Name}' AND CHECK_ITEM='{Check_Item}'";
        PubClass.getdatatablenoreturnMES(sSQL);



        sSQL = $"DELETE FROM  SAJET.SYS_PROJECT_MODEL_BASE WHERE PROJECT_ID='{NUMBER_ID}'AND MODEL_NAME='{ModelName}' AND PDLINE_NAME='{PdlineName}' AND  CHECK_NAME='{Check_Name}' AND CHECK_ITEM='{Check_Item}'";
        PubClass.getdatatablenoreturnMES(sSQL);
    ...
    public string GetProcessL(string ProjectName, string Check_Name)
    {
        ...
        string PROJECT_ID = dt.Rows[0]["PROJECT_ID"].ToString();
        sSQL = $"SELECT DISTINCT CHECK_ITEM FROM SAJET.SYS_ECHECK_PROJECT_BASE WHERE PROJECT_ID='{PROJECT_ID}' AND CHECK_NAME='{Check_Name}'";
        dt = PubClass.getdatatableMES(sSQL);
        return JsonConvert.SerializeObject(dt);
    }
    ...
    public string show(string PdlineName, string ProjectName, string ModelName)
    {
        ...
        string SQL = $"SELECT A.STATUS,A.MODEL_NAME,A.PROJECT_ID,A.PROJECT_NAME,A.PDLINE_NAME,A.FREQUENCY,A.CHECK_NAME,A.CHECK_ITEM,A.UNIT,A.CHECK_QTY,A.UPPER,A.FLOOR,B.EMP_NAME,A.create_date,c.EMP_NAME AS EMP_NAME1,A.UPDATE_date FROM SAJET.SYS_PROJECT_MODEL_BASE A,SAJET.SYS_EMP B,SAJET.SYS_EMP C " +
