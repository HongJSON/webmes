...
public class E09 : IHttpHandler
{
    ...
    public string ShowDown(string PdlineName, string ModelName, string dataType, string datetimepicker)
    {
        ...
        LOG += $"</tr>";
        string LOG1 = $"<tr style=text-align:center><td colspan='1'>机种名称</td><td colspan='{P-3}'>{ModelName}</td><td colspan='3'>日期</td><td colspan='3'>{datetimepicker}</td><td colspan='2'>版本</td><td colspan='2'>A</td></tr>";
        string LOG2 = $"<tr style=text-align:center><td colspan='1'>线别</td><td colspan='{P-3}'>{PdlineName}</td><td colspan='3'>班别</td><td colspan='3'>{dataType}</td><td colspan='2'>成品料号</td><td colspan='2'></td></tr>";
        ...
        string LOG3 = $"<tr style=text-align:center><td colspan='1'>制程名称</td><td colspan='2'>检查事项-Check Point</td><td colspan='1'>抽测数量</td><td colspan='1'>管控上限</td><td colspan='1'>管控下限</td>"+LOG;
        ...
        //for (int K = 0; K <= dt.Rows.Count - 1; K++)
        {
            ...
            for (int T = 0; T <= DTCOUNT.Rows.Count - 1; T++)
            {
                ...
                LOG5 += $"<td  colspan='1'>{DTCOUNT.Rows[T]["CHECK_NAME"].ToString()}</td><td colspan='1'>{TYPE1}</td><td colspan='1'>{DTCOUNT.Rows[T]["CHECK_QTY"].ToString()}</td><td colspan='1'>{DTCOUNT.Rows[T]["UPPER"].ToString()}</td><td colspan='1'>{DTCOUNT.Rows[T]["FLOOR"].ToString()}</td>";




                SQL = $@"SELECT DISTINCT FREQUENCY FROM   SAJET.SYS_PROJECT_MODEL_BASE WHERE PROJECT_ID='{DTCOUNT.Rows[T]["PROJECT_ID"].ToString()}'AND MODEL_NAME='{ModelName}' AND PDLINE_NAME='{PdlineName}'";
                ...
                for (int Q = 0; Q <= dt1.Rows.Count - 2; Q++)
                {
                    ...
                    SQL = $"SELECT B.{TYPE} FROM SAJET.G_CHECK_WORK_BASE A,SAJET.G_CHECK_WORK_BASE_INFO B WHERE A.WORK_DATE='{datetimepicker}' AND A.WORK_TYPE='{dataType}' AND A.PDLINE_NAME='{PdlineName}' " +
                          $"AND A.MODEL_NAME='{ModelName}' AND A.WORK_NO=B.WORK_NO AND B.CHECK_NAME='{DTCOUNT.Rows[T]["CHECK_NAME"].ToString()}' AND B.CHECK_ITEM='{DTCOUNT.Rows[T]["CHECK_ITEM"].ToString()}' AND B.{TYPE} IS NOT NULL ";
                ...
                SQL = $"SELECT B.{LTYPE} FROM SAJET.G_CHECK_WORK_BASE A,SAJET.G_CHECK_WORK_BASE_INFO B WHERE A.WORK_DATE='{datetimepicker}' AND A.WORK_TYPE='{dataType}' AND A.PDLINE_NAME='{PdlineName}' " +
                          $"AND A.MODEL_NAME='{ModelName}' AND A.WORK_NO=B.WORK_NO AND B.CHECK_NAME='{DTCOUNT.Rows[T]["CHECK_NAME"].ToString()}' AND B.CHECK_ITEM='{DTCOUNT.Rows[T]["CHECK_ITEM"].ToString()}' AND B.{LTYPE} IS NOT NULL ";
        ...
        if (dt.Rows.Count == 0)
        {
            ...
            for (int i = 0; i <= dt.Rows.Count - 1; i++)
            {
                ...
                if (dt1.Rows.Count > 0)
                {
                    ...
                    SQL = $"INSERT INTO SAJET.G_CHECK_WORK_BASE_INFO(WORK_NO,PROJECT_ID,PROJECT_NAME,PDLINE_NAME,FREQUENCY,CHECK_NAME,CHECK_ITEM,UNIT,CHECK_QTY,UPPER,FLOOR,NUMBER_ID,MODEL_NAME,ENABLED,CREATE_DATE,CREATE_EMPID)" +
                    $"SELECT '{WORK_NO}','{PROJECT_ID}','{ProjectName}','{PdlineName}','{FREQUENCY}','{CHECK_NAME}','{CHECK_ITEM}','{UNIT}','{CHECK_QTY}','{UPPER}','{FLOOR}','{NUMBER_ID}','{ModelName}','Y',SYSDATE,'{userEmpId}' FROM DUAL";
                    PubClass.getdatatablenoreturnMES(SQL);
                }
            }
        }
