...
public class E03 : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        switch (funcName)
        {
            ...
            case "addTemplateInfo":
                ...
                break;
            case "addTemplateInfo1":
                rtnValue = addTemplateInfo1(Section_Name, user, Project_Name, Check_No, Check_Name, Check_Item);
                break;
            case "del":
            ...
            case "excel":
                ...
                break;
            case "ShowDown":
                rtnValue = ShowDown(Project_Name);
                break;
    ...
    public string addTemplateInfo1(string Section_Name, string user, string Project_Name, string Check_No, string Check_Name, string Check_Item)
    {
        ...
        sSQL = $"UPDATE SAJET.SYS_ECHECK_PROJECT_BASE SET UPDATE_DATE=SYSDATE,UPDATE_EMPID='{userEmpId}',SECTION_NAME='{Section_Name}',CHECK_NO='{Check_No}' WHERE NUMBER_ID='{NUMBER_ID}'";
        ...
        sSQL = $"UPDATE  SAJET.SYS_PROJECT_MODEL_BASE SET CHECK_NO='{Check_No}',UPDATE_EMPID='{userEmpId}',UPDATE_DATE=SYSDATE WHERE PROJECT_ID='{PROJECT_ID}'AND CHECK_NAME='{Check_Name}'AND CHECK_ITEM='{Check_Item}'";
        PubClass.getdatatablenoreturnMES(sSQL);

        sSQL = $"INSERT INTO SAJET.SYS_HT_PROJECT_MODEL_BASE SELECT * FROM SAJET.SYS_PROJECT_MODEL_BASE WHERE PROJECT_ID='{PROJECT_ID}' AND  CHECK_NAME='{Check_Name}' AND CHECK_ITEM='{Check_Item}'";
        PubClass.getdatatablenoreturnMES(sSQL);
    ...
    public string ShowTemplate1(string JJ)
    {
        string sSQL = $@"SELECT PROJECT_NAME FROM  SAJET.SYS_CHECK_PROJECT_CONFIG WHERE PROJECT_NAME LIKE'" + JJ + "%' ORDER BY  PROJECT_NAME";
    ...
    public string ShowDown(string Project_Name)
    {
        ...
        string sSQL = $@"SELECT A.NUMBER_ID,A.PROJECT_ID,B.PROJECT_NAME,A.CHECK_NO,A.CHECK_NAME,A.CHECK_ITEM,A.SECTION_NAME,C.EMP_NAME,D.EMP_NAME AS EMP_NAME1,A.CREATE_DATE,A.UPDATE_DATE FROM SAJET.SYS_ECHECK_PROJECT_BASE A,SAJET.SYS_CHECK_PROJECT_CONFIG B,SAJET.SYS_EMP C,SAJET.SYS_EMP D
                  WHERE A.PROJECT_ID=B.PROJECT_ID
                  AND A.CREATE_EMPID=C.EMP_ID(+)
                  AND A.UPDATE_EMPID=D.EMP_ID(+)" + sql + "  ORDER BY B.PROJECT_NAME,TO_NUMBER(A.CHECK_NO)";
        DataTable dt = PubClass.getdatatableMES(sSQL);
        string tableHtml = "<html><head><meta charset='UTF-8'></head><body><table border=1>";
        string html = $"<tr style=text-align:center ><td style=text-align:center colspan='1'>制程名称</td><td style=text-align:center colspan='1'>事项序号</td><td style=text-align:center colspan='1'>点检制程名称</td><td style=text-align:center colspan='1'>检查事项</td><td style=text-align:center colspan='1'>部门</td></tr>";
        for (int i = 0; i <= dt.Rows.Count - 1; i++)
        {
            html += $"<tr style=text-align:center ><td style=text-align:center colspan='1'>{dt.Rows[i]["PROJECT_NAME"].ToString()}</td><td style=text-align:center colspan='1'>{dt.Rows[i]["CHECK_NO"].ToString()}</td><td style=text-align:center colspan='1'>{dt.Rows[i]["CHECK_NAME"].ToString()}</td><td style=text-align:center colspan='1'>{dt.Rows[i]["CHECK_ITEM"].ToString()}</td><td style=text-align:center colspan='1'>{dt.Rows[i]["SECTION_NAME"].ToString()}</td></tr>";
        }
        tableHtml += html + "</table></body></html>";
            ...
            if (context.Request.Files.Count > 0)
            {
                ...
                for (int i = 0; i < files.Count; i++)
                {
                    ...
                    for (int L = 1; L < worksheet.Cells.LastRowIndex + 1; L++)
                    {
                        ...
                        //{
                        //    return "[{\"ERR_CODE\":\"Y\",\"ERR_MSG\":\"NG,第" + (L+1) + "行存在重复数据!\"}]";
                        ...
                        if (dt.Rows.Count == 0)
                        {
                            return "[{\"ERR_CODE\":\"Y\",\"ERR_MSG\":\"NG,第" + (L + 1) + "行制程名称不存在!\"}]";
                        }
                        string PROJECT_ID = dt.Rows[0]["PROJECT_ID"].ToString();
                        ...
                        if (dt.Rows.Count == 0)
                        {
                            return "[{\"ERR_CODE\":\"Y\",\"ERR_MSG\":\"NG,第" + (L + 1) + "行部门信息不存在!\"}]";
                        }
                        ...
                        if (dt.Rows.Count > 0)
                        {
                            //return "[{\"ERR_CODE\":\"Y\",\"ERR_MSG\":\"NG,第" +(L+1) + "行事项已维护过!\"}]";
                            continue;
                        }
                    ...
                    for (int L = 1; L < worksheet.Cells.LastRowIndex + 1; L++)
                    {
                        ...
                        sSQL = $"UPDATE  SAJET.SYS_PROJECT_MODEL_BASE SET CHECK_NO='{Check_No}' WHERE PROJECT_ID='{PROJECT_ID}'AND CHECK_NAME='{Check_Name}'AND CHECK_ITEM='{Check_Item}'";
                        PubClass.getdatatablenoreturnMES(sSQL);
