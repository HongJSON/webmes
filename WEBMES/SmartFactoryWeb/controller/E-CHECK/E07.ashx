...
public class E07 : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        switch (funcName)
        {
            ...
            case "ShowTemplateData":
                rtnValue = ShowTemplateData(DN_NO,Frequency);
                break;
            case "ShowTemplateData1":
                rtnValue = ShowTemplateData1(DN_NO);
                break;
            case "ShowFre":
                rtnValue = ShowFre(DN_NO);
                break;
    ...
    public string ShowTemplateData1(string DN_NO)
    {
        ...
        sSQL = $"SELECT DISTINCT A.FREQUENCY,C.WORK_TYPE FROM SAJET.G_CHECK_WORK_BASE_INFO A,SAJET.G_CHECK_FACA B,SAJET.G_CHECK_WORK_BASE C WHERE A.WORK_NO='{DN_NO}'AND A.WORK_NO=C.WORK_NO AND A.WORK_NO=B.WORK_NO AND A.NUMBER_ID=B.NUMBER_ID AND A.ENABLED='Y' AND B.ENABLED='Y'";
        dt = PubClass.getdatatableMES(sSQL);
        return "[{\"ERR_CODE\":\"N\",\"ERR_MSG\":\"" + dt.Rows.Count + "\",\"ERR_MSG1\":\"" + dt.Rows[0]["WORK_TYPE"].ToString() + "\"}]";

    }
    public string ShowFre(string DN_NO)
    {
        string sSQL = $@"SELECT * FROM  SAJET.G_CHECK_WORK_BASE   WHERE  WORK_NO='{DN_NO}'";
        DataTable dt = PubClass.getdatatableMES(sSQL);
        ...
        sSQL = $"SELECT DISTINCT A.FREQUENCY FROM SAJET.G_CHECK_WORK_BASE_INFO A,SAJET.G_CHECK_FACA B WHERE  A.WORK_NO='{DN_NO}'AND A.WORK_NO=B.WORK_NO  AND A.NUMBER_ID=B.NUMBER_ID AND A.ENABLED='Y' AND B.ENABLED='Y'";
        dt = PubClass.getdatatableMES(sSQL);
    ...
    public string ShowTemplateData(string DN_NO,string Frequency)
    {
        string sSQL = $@"SELECT * FROM  SAJET.G_CHECK_WORK_BASE   WHERE  WORK_NO='{DN_NO}'";
        DataTable dt = PubClass.getdatatableMES(sSQL);
        ...
        sSQL = $@"SELECT CHECK_TIME FROM  SAJET.SYS_CHECK_PROJECT_TITLE WHERE PROJECT_ID='{PROJECT_ID}' AND  FREQUENCY='{Frequency}'ORDER BY SEQ";
        dt = PubClass.getdatatableMES(sSQL);
        return JsonConvert.SerializeObject(dt);


    }
    public string show(string datetimepicker)
    {
        string SQL = $"SELECT A.* FROM SAJET.G_CHECK_WORK_BASE_INFO A,SAJET.G_CHECK_FACA B WHERE A.WORK_DATE='{datetimepicker}' AND A.NUMBER_ID=B.NUMBER_ID   AND A.ENABLED='Y' AND B.ENABLED='Y'";
    ...
    public string UpdateTemplateInfo1(string USERTYPE, string DN_NO, string user, string ID, string Date, string Datew, string wf, string Pdline, string Class, string Terminal, string Abnormal, string Analysis, string Improvement, string Category, string Person, string Director, string StartTime, string FinishTime, string Followup, string Status, string Auditors)
    {
        ...
        if (USERTYPE == "点检")
        {
            ...
            string sSQL = $@"UPDATE SAJET.G_CHECK_FACA SET DATED='{Date}',DATEW='{Datew}',WF='{wf}',ABNORMAL='{Abnormal}',PDLINE_NAME='{Pdline}',PROCESS_NAME='{Terminal}',CATEGORY='{Category}',PERSON='{Person}',DIRECTOR='{Director}',FINISHTIME='{FinishTime}',FOLLOWUP='{Followup}',STATUS='{Status}',AUDITORS='{Auditors}',UPDATE_DATE=SYSDATE,UPDATE_USERID='{userEmpId}'WHERE WORK_NO='{DN_NO}' AND  NUMBER_ID='{ID}'";
